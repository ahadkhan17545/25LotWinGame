...
const userInfo = async (req, res) => {
  ...
  const [recharge] = await connection.query(
    "SELECT * FROM `recharge` WHERE `phone` = ? AND `status` = 0",
    [user?.phone],
  );
...
//   ]);
  ...
  // Retrieve the updated money field
  const [result] = await connection.query(
    "SELECT * FROM users WHERE `phone` = ?",
    [phone],
  );
  const update_feild = result[0];

  return res.status(200).json({
    message: "Success",
    status: true,
    data: {
      code: update_feild?.code,
      id_user: update_feild?.id_user,
      name_user: update_feild?.name_user,
      phone_user: update_feild?.phone,
      money_user: update_feild?.money,
      bonus_money: update_feild?.bonus_money,
      avatar: update_feild?.avatar,
      level: update_feild?.level,
      total_withdraw: totalWithdraw,
      total_recharge: totalRecharge,
      vip_level: update_feild?.vip_level,
      withdrawable_amount: update_feild?.withdrawable_money,
    },
  ...
  try {
    ...
    users.forEach(async (user) => {
      const [members] = await connection.execute(
        "SELECT * FROM users WHERE invite = ? AND is_active = 1",
        [user.code],
      );
      ...
      if (activeMembersCount > 0) {
        ...
        const userPhones = [user.phone, ...mems]
          .map((mem) => mem.phone.toString())
          .join(",");
        ...
        if (betUsers.length > 0 && betUsers[0].totalAmount) {
          totalBetAmount = betUsers[0].totalAmount;
        }
        ...
        if (
          ...
          // get all members today recharge
          // const [todaySubordinateRecharges] = await connection.execute(
          //   `SELECT phone as user_phone, SUM(money) as recharge_amount FROM recharge WHERE phone in (${members.map(() => "?").join(",")}) AND today = DATE(STR_TO_DATE(today, '%Y-%d-%m %h:%i:%s %p')) = CURDATE() GROUP BY phone`,
  ...
  try {
    ...
    users.forEach(async (user) => {
      ...
      if (activeMembersCount > 0) {
        ...
        } else if (activeMembersCount >= 1000 && activeMembersCount < 2000) {
          teamBonusObj = teamBonusMap[1000];
        } else if (activeMembersCount >= 2000 && activeMembersCount < 5000) {
          teamBonusObj = teamBonusMap[2000];
        } else if (activeMembersCount >= 5000 && activeMembersCount < 10000) {
        ...
        if (teamBonusObj) {
          ...
          // DATE(STR_TO_DATE(today, '%Y-%d-%m %h:%i:%s %p')) = CURDATE() AND
          const userPhones = mems.map((mem) => mem.phone.toString()).join(",");
          ...
          for (let i = 0; i < deposits.length; i++) {
            if (!(deposits[i].total_amount || 0) < teamBonusObj.deposit) {
              bonusEligible = false;
            }
          }
    ...
    console.log("Error from distribueTeamBonuses: " + err);
  }
};
...
const distributeDailyInterest = async () => {
  try {
    ...
    users.forEach(async (user) => {
      ...
      if (parseFloat(user.money) > 0) {
        let bonus = parseFloat(
          (parseFloat(user.money) * parseFloat(process.env.DAILY_INTEREST)) /
            100,
        );
    ...
    console.log("Error from distributeDailyInterest: " + err.toString());
  }
};
...
const distributeExtraSalaryDaily = async () => {
  try {
    ...
    users.forEach(async (user) => {
      ...
      // check if today refferal total is 3 or geater and their recharge amount is greater than 0
      if (refUsersCount >= 3 && totalRechargeAmountForToday > 0) {
        const elilibleSalaryObject = extraSalaryList.find(
          (item) =>
            totalAmountOfRechargeOfToday >= item.minimumTotalRechargeForTheDay,
        );
    ...
    console.log("error form distributeExtraSalaryDaily:");
  }
};
const distributeMonthlyLevelIncome = async () => {
  try {
    ...
    while (true) {
      // get recharges which are 30 days old
      const [unsettledRecharges] = await connection.execute(
        `SELECT * FROM recharge WHERE level_income_settled = 0 AND DATE(FROM_UNIXTIME(time / 1000)) >= CURDATE() - INTERVAL 30 DAY LIMIT ${batchSize} OFFSET ${offset}`,
      );

      if (unsettledRecharges.length === 0) break;

      for (const recharge of unsettledRecharges) {
        ...
        if (parentMembers.length > 0) {
          const comissAmount =
            parseFloat(recharge.money) / 222.2222222 / parentMembers.length;

          const commissionsToInsert = parentMembers.map((parentMember) => [
            generateCommissionId(),
            parentMember.phone,
            userInfo.phone,
            comissAmount,
            parseInt(parentMember.level) - 1,
            Date.now(),
          ]);
          ...
          await Promise.all(
            parentMembers.map((parentMember) =>
              updateUserMoney(parentMember.phone, comissAmount),
            ),

            await connection.execute(
              "UPDATE recharge SET level_income_settled = 1 WHERE id = ?",
              [recharge.id],
            ),
          );
        }
    ...
    console.log("error form distributeMonthlyLevelIncome:" + err.toString());
  }
};
