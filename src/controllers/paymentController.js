...
const initiateManualUPIPayment = async (req, res) => {
  ...
  const upiInitiationData = {
    icepay: {
      imgKey: "icepay_scan",
      upiKey: "icepay_upi",
    },
    sevendays: {
      imgKey: "sevendays_scan",
      upiKey: "sevendays_upi",
    },
    pailepay: {
      imgKey: "pailepay_scan",
      upiKey: "pailepay_upi",
    },
    rspay: {
      imgKey: "rspay_scan",
  ...
  const platform = upiInitiationData[req.query.platform];
  if (!platform) {
    return;
  }
  return res.render("wallet/manual_payment.ejs", {
    Amount: query?.am,
    UpiId: row[upiInitiationData[req.query.platform].upiKey],
    scan: row[upiInitiationData[req.query.platform].imgKey],
  });
};
...
const addManualUPIPaymentRequest = async (req, res) => {
  try {
    ...
    const newRecharge = {
      ...
      utr: utr,
      phone: user.phone,
      money: money + (money >= 1000 ? money * 0.05 : 0),
