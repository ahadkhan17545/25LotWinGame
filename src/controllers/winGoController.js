...
const addWinGo = async (game) => {
  try {
    ...
    if (isPendingGame) {
      ...
      if (minPlayers.length >= 2) {
        ...
        const categories = await Promise.all(
          betColumns.map(async (column) => {
            ...
            return {
              name: column.name,
              total_money: parseFloat(result[0]?.total_money) || 0,
            };
          }),
        );
        ...
        const listOfEmptyBetCategories = categories.filter(
          (item) => item.total_money == 0,
        );

        const emptyBetNumbers = listOfEmptyBetCategories.reduce(
          (total, cat) => [...total, ...colorBets[cat.name]], // Access colorBets using `cat.name`
          [], // Start with an empty array
        );

        console.log({ listOfEmptyBetCategories });
        ...
        const [lastGameWingo] = await connection.execute(
          "SELECT * FROM wingo WHERE game = ? AND status = 1 ORDER BY id DESC LIMIT 1",
          [join],
        );

        if (lastGameWingo.length > 0) {
          let targetValue = lastGameWingo[0]?.amount;
          let randomValue;
          do {
            randomValue =
              emptyBetNumbers[
                Math.floor(Math.random() * emptyBetNumbers.length)
              ];
          } while (targetValue == randomValue);

          amount = randomValue;
        } else {
          amount =
            emptyBetNumbers[Math.floor(Math.random() * emptyBetNumbers.length)];
        }
      }
      ...
      if (nextResult == "-1") {
        // game algorithm generate result
        console.log("Upading result: " + amount);
        await connection.query(
          "UPDATE wingo SET amount = ?, status = 1, release_status = 1 WHERE period = ? AND game = ?",
          [amount, previousPeriod, join],
        );
...
const handlingWinGo1P = async (typeid) => {
  try {
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      let totals =
        parseFloat(users[0].money) + parseFloat(winAmount);
