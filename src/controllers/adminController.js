...
const settings = async (req, res) => {
  ...
  return res.render("manage/settings.ejs", {
    qr_img: "/" + row.usdt_scan,
    icepay_img: "/" + row.icepay_scan,
    sevendays_img: "/" + row.sevendays_scan,
    pailepay_img: "/" + row.pailepay_scan,
    rspay_img: "/" + row.rspay_scan,
    ...row,
  });
};
// xác nhận admin
...
const updateUserBank = async (req, res) => {
  ...
  if (usdtAccount) {
    console.log(`${req.body.usdtAddress}, ${req.body.addressAlias}`);
    await connection.execute(
...
//   if (totalRecharge === 1) {
    ...
    parentMembers.forEach(async (member) => {
      const bonus = bonuses.find((it) => it.level === member.level);
      if (bonus) {
        const bonusMoney = (Number(money) * bonus.bonusInPercentage) / 100;

        await connection.query(
          "UPDATE users SET money = money + ?, total_money = total_money + ? WHERE `phone` = ?",
          [bonusMoney, bonusMoney, member.phone],
        );
        await addRewards(
          member.phone,
          bonusMoney,
          REWARD_TYPES_MAP.REFERRAL_DEPOSIT_BONUS,
        );
      }
    });
//   }
...
const updateQrcodeImage = async (req, res, next) => {
  ...
  const { icepay_upi, sevendays_upi, pailepay_upi, rspay_upi } = req.body;

  await connection.execute(
    `UPDATE admin_ac SET icepay_upi = ?, sevendays_upi = ?,  pailepay_upi = ?, rspay_upi = ? WHERE id = 1`,
    [
      icepay_upi || "",
      sevendays_upi || "",
      pailepay_upi || "",
...
const exportWithdrawList = async (req, res) => {
  try {
    ...
    const csv = json2csvParser.parse(filteredList);
...
const updateQrCodes = async (req, res, next) => {
  ...
  fs.writeFile(filepath, req.file.buffer, async (err) => {
    ...
    console.log({ filename, parsed, index, oldFile });
    await connection.execute(
      `UPDATE admin_ac SET ${column} = '${JSON.stringify(parsed)}' WHERE id = 1`,
      [filename],
    );
